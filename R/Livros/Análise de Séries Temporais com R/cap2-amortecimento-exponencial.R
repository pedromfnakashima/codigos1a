...
# Assembleia
setwd("C:\\Users\\Pedro\\Desktop\\Pedro Nakashima\\Códigos, Dados, Documentação e Cheat Sheets")
# Casa
# --------
getwd()getwd()

setwd(".\\Cursos e Livros\\Análise de Séries Temporais com R\\cap2-HW")
...
bd_elecequip <- elecequip
library(lubridate)
library(xts)
library(dplyr)

ts <- as.xts(bd_elecequip)
tsdf <- data.frame(year = year(ts), month = month(ts), elecequip=ts)


write.csv2(tsdf, file="elecequip.csv", sep=";", dec=",", row.names = FALSE)
