# -*- coding: utf-8 -*-
"""
Created on Sun Dec 27 19:29:54 2020
...
""" Muda o diretório para que seja possível a importação das funções nos arquivos f_*.py"""
caminho_wd = caminho_base / 'codigos_versionados' / 'Python' / 'Economia' / 'Siconfi' # pasta onde tem os scripts (para importar as funções)
os.chdir(caminho_wd)
...
def gd_siconfi_muns(início, final, deflaciona=False):
    ...
    li_períodos = g_li_datas(início=início, final=final, extensão='.zip')
    ...
    if deflaciona == True:
        # A função abaixo tem que estar em arquivo no mesmo diretório (e também ser wd)
        from f_deflacionaDic import deflaciona_dic
        dicionário_completo = deflaciona_dic(dicionário_completo, base='último', exclusões=['MR'], download=False)
    
    return dicionário_completo


dic_muns =  gd_siconfi_muns(início='2019b1', final='2020b2', deflaciona=False)

dic_muns_deflacionado =  gd_siconfi_muns(início='2019b1', final='2020b2', deflaciona=True)
